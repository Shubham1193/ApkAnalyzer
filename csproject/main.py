...
# Getting the list of all the apps installed on device
...
# Getting the exact path of the apk in device
...
# Pulling the app from the phone
...
# Parsing the manifest file of the extracted file
...
# Sending the manifest file data to gemini server
...
# On button click from static page 1st get apk path . 2nd pull apk . 3rd decode apk . 4th parse_manifest . 5 send the data to gemini api .6 remove the apk and files  
...
def show_static_analysis(apps):
    ...
    for index, app in enumerate(apps, start=1):
        last_element = app.rsplit(".", 1)[-1] 
        scroll_text.insert(tk.END, f"{index} - {last_element}\n")
...
def show_initial_page():
    ...
    main_label.pack(pady=40 , padx=-34)
